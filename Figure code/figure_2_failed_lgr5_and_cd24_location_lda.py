...
def main():
    ...
    ax_enterocyteness.scatter(points.x_values, points.y_values, c=points.enterocyteness_values,
                              cmap=_create_colormap("ENTEROCYTE"), vmin=0, vmax=1, s=12, lw=0)
    ax_enterocyteness.set_title("Enterocyte probability")

    # Plot LGR5
    min_lgr5 = numpy.percentile(points.lgr5_values, 1)
    max_lgr5 = numpy.percentile(points.lgr5_values, 99)
    ax_lgr5.scatter(points.x_values, points.y_values, c=points.lgr5_values, cmap="Greens", vmin=min_lgr5, vmax=max_lgr5, s=12, lw=0)
    ax_lgr5.set_title("Lgr5 intensity")

    # Plot CD24
    min_cd24 = numpy.percentile(points.cd24_values, 1)
    max_cd24 = numpy.percentile(points.cd24_values, 99)
    ax_cd24.scatter(points.x_values, points.y_values, c=points.cd24_values, cmap="Reds", vmin=min_cd24, vmax=max_cd24, s=12, lw=0)
...
def _extract_lda_data(adata: AnnData, lda: LinearDiscriminantAnalysis, experiments: List[Experiment]) -> _Points:
    ...
    for experiment in experiments:
        ...
        time_point = experiment.positions.last_time_point()
        for position in experiment.positions.of_time_point(time_point):
            ...
            intensity_cd24 = 1 if experiment.position_data.get_position_data(position, "cd24_positive") else 0#intensity_calculator.get_normalized_intensity(experiment, position, intensity_key=_INTENSITY_KEY_CD24)
