...
_CHANCE_BINS = 50
_SIGNAL_BINS = 50
_COLORMAP = matplotlib.colors.LinearSegmentedColormap.from_list("OurGreens", ["#80C97F", "#00441B"])
...
def _get_intensity_against_paneth_chance_single_experiment(experiment: Experiment, offset_h: float
    ...
    paneth_index = experiment.global_data.get_data("ct_probabilities").index("STEM")
    cd24_values = list()
    panethness_values = list()

    resolution = experiment.images.resolution()
    time_point_divider = int(0.5 * _LGR5_AVERAGING_WINDOW_H / resolution.time_point_interval_h)
