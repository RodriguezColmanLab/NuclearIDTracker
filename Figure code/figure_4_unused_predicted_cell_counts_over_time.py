...
def _plot(figure: Figure, paneth_cells_ablation: Dict[str, _PredictedCellcount], paneth_cells_regeneration: Dict[str, _PredictedCellcount]):
    ...
    ax_enterocyte.set_ylabel("Cell count in organoid")
    ...
    for i, name in enumerate(useable_experiments):
        ...
        ax_paneth.plot(times_h, paneth_cell_counts, color=_PANETH_COLORS[i], linewidth=2)
        ax_stem.plot(times_h, stem_cell_counts, color=_STEM_COLORS[i], linewidth=2)
        ax_enterocyte.plot(times_h, enterocyte_cell_counts, color=_ENTEROCYTE_COLORS[i], linewidth=2)
