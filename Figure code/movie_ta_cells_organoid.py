...
_OUTPUT_FILE = "E:/Scratch/Figures/movie_ta_{experiment}.tif"

_NUCLEUS_CHANNEL = ImageChannel(index_zero=0)
_SEGMENTATION_CHANNEL = ImageChannel(index_zero=2)

_COLORMAP = matplotlib.colormaps.get_cmap("gnuplot2")
...
def _get_cell_types_image_rgb(experiment: Experiment, time_point: TimePoint, z: int, segmentation_image: Image):
    ...
    for region in skimage.measure.regionprops(segmentation_image.array):
        ...
        # Find back the cell on the stem-to-ec scale
        max_cell_type = cell_types[numpy.argmax(probabilities)]
        if max_cell_type not in {"STEM", "ENTEROCYTE"}:
            continue  # Only color cells on the stem-to-enterocyte scale
        stem_to_ec_scale = 0
        for cell_type, probability in zip(cell_types, probabilities):
            if cell_type == "STEM":
                stem_to_ec_scale += probability
            elif cell_type == "ENTEROCYTE":
                continue
            else:
                stem_to_ec_scale += probability / 2

        # We define a TA score that is 1 when the stem cell probability is 0.5, and 0 when it is 0.4 or 0.6
        distance_to_05 = abs(stem_to_ec_scale - 0.5)
        ta_score = _clip(1 - distance_to_05 * 20)
        color = _COLORMAP(ta_score)[0:3]
