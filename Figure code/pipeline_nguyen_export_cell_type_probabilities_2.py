...
def _export_tracks(experiment: Experiment, output_file: str, included_tracks: Iterable[LinkingTrack]):
    ...
    for track in included_tracks:
        ...
        table.set_cell(1, working_column + 3 + len(cell_types), "Cell type")
        for position in track.positions():
            ...
            if ct_probabilities is None:
                ...
                final_cell_type = None
            else:
                final_cell_type = cell_types[numpy.argmax(ct_probabilities)]
            ...
            table.set_cell(position.time_point_number() - first_time_point_number + 2, working_column + 3 + len(cell_types), str(final_cell_type))

        working_column += 4 + len(cell_types)
...
def main():
    ...
    export_unfiltered(_DATA_FILE_REGENERATION, "Stem cell regeneration v2")
    export_unfiltered(_DATA_FILE_CONTROL, "Controls for stem cell regeneration v2")

    # Now do the filtered files
    export_filtered(_DATA_FILE_REGENERATION, "Stem cell regeneration (filtered) v2")
    export_filtered(_DATA_FILE_CONTROL, "Controls for stem cell regeneration (filtered) v2")
