...
class CellTypeTransitionMatrix:
    ...
    def __init__(self):
        self._count_matrix = numpy.zeros((_STEM_TO_ENTEROCYTE_BINS + 3, _STEM_TO_ENTEROCYTE_BINS + 3),
                                         dtype=numpy.float32)
    ...
    def add_position(self, cell_types: List[str], probabilities_start: List[float], probabilities_end: List[float]):
        ...
        if stem_to_ec_position_start is not None:
            bin_start = 2 + int(stem_to_ec_position_start * _STEM_TO_ENTEROCYTE_BINS)
        if stem_to_ec_position_end is not None:
            bin_end = 2 + int(stem_to_ec_position_end * _STEM_TO_ENTEROCYTE_BINS)
    ...
    def get_transition_count(self, from_cell_count: CellTypeInMatrix, to_cell_type: CellTypeInMatrix) -> int:
        ...
        bin_start_1 = 2 + int(from_cell_count.stem_to_ec_start * _STEM_TO_ENTEROCYTE_BINS)
        bin_start_2 = 2 + int(from_cell_count.stem_to_ec_end * _STEM_TO_ENTEROCYTE_BINS)
        bins_start = list(range(bin_start_1, bin_start_2 + 1))
        if from_cell_count.includes_paneth:
            bins_start.append(0)

        bin_end_1 = 2 + int(to_cell_type.stem_to_ec_start * _STEM_TO_ENTEROCYTE_BINS)
        bin_end_2 = 2 + int(to_cell_type.stem_to_ec_end * _STEM_TO_ENTEROCYTE_BINS)
...
def main():
    transition_matrix_control = calculate_transition_matrix(_DATA_FILE_CONTROL)
    transition_matrix_regeneration = calculate_transition_matrix(_DATA_FILE_REGENERATION)

    figure = lib_figures.new_figure(size=(9, 3.5))
    ax_control, ax_regeneration, ax_scalebar = figure.subplots(nrows=1, ncols=3, width_ratios=[1, 1, 0.05])

    ax_control.set_title("Control")
    _plot_transition_matrix(ax_control, transition_matrix_control)

    ax_regeneration.set_title("Regeneration")
    scaleable = _plot_transition_matrix(ax_regeneration, transition_matrix_regeneration)
    figure.colorbar(scaleable, cax=ax_scalebar).set_label("Cell count")
    figure.tight_layout()
    plt.show()


def _plot_transition_matrix(ax: Axes, transition_matrix: CellTypeTransitionMatrix):
    x_ticks = [0] + list(numpy.arange(_STEM_TO_ENTEROCYTE_BINS + 1) + 1.5)
    y_ticks = [0] + list(numpy.arange(_STEM_TO_ENTEROCYTE_BINS + 1) + 1.5)
    x_ticklabels = ["Paneth"] + [(f"{i / _STEM_TO_ENTEROCYTE_BINS:.2f}" if i % 2 == 0 else "") for i in
                                 range(_STEM_TO_ENTEROCYTE_BINS + 1)]
    y_ticklabels = ["Paneth"] + [(f"{i / _STEM_TO_ENTEROCYTE_BINS:.2f}" if i % 2 == 0 else "") for i in
                                 range(_STEM_TO_ENTEROCYTE_BINS + 1)]
    ...
    ax.set_ylim(_STEM_TO_ENTEROCYTE_BINS + 1.5, -0.5)
    ax.set_xlim(_STEM_TO_ENTEROCYTE_BINS + 1.5, -0.5)
    ax.axhline(y=_STEM_TO_ENTEROCYTE_BINS * .5 + 1.5, color="white", linewidth=2)
    ax.axvline(x=_STEM_TO_ENTEROCYTE_BINS * .5 + 1.5, color="white", linewidth=2)
    ...
    for x in range(count_matrix.shape[1]):
        for y in range(count_matrix.shape[0]):
            ...
            ax.text(x, y, f"{count_matrix[y, x]:.0f}", ha="center", va="center",
                    color="white" if count_matrix[y, x] < 20 else "black")

    # Create color image for the cell types
    color_image = numpy.zeros((_STEM_TO_ENTEROCYTE_BINS, 1, 3), dtype=numpy.float32)
    for i in range(_STEM_TO_ENTEROCYTE_BINS):
        color_image[i, 0, :] = lib_figures.get_stem_to_ec_color(i / _STEM_TO_ENTEROCYTE_BINS)
    ax.imshow(color_image, extent=(
        _STEM_TO_ENTEROCYTE_BINS + 1.5, _STEM_TO_ENTEROCYTE_BINS + 0.5, _STEM_TO_ENTEROCYTE_BINS + 1.5, 1.5),
              aspect="auto")
    ax.imshow(color_image.reshape((1, _STEM_TO_ENTEROCYTE_BINS, 3)), extent=(
        1.5, _STEM_TO_ENTEROCYTE_BINS + 1.5, _STEM_TO_ENTEROCYTE_BINS + 1.5, _STEM_TO_ENTEROCYTE_BINS + 0.5),
              aspect="auto")
    ax.set_aspect("equal")
